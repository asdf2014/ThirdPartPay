...
class Themify_Border_Control extends Themify_Control
{
    ...
    <?php endif; ?>
        ...
        foreach ($sides as $side => $side_label) : ?>
            <div class="themify-customizer-brick <?php if ($first) : echo 'useforall';
            else : echo 'component'; endif; ?>">
                <div
                    class="wide-label <?php if ($first) : echo 'same-label'; endif; ?>" <?php if ($first) : echo 'data-same="' . esc_attr__('Border', 'themify') . '" data-notsame="' . esc_attr($side_label) . '"'; endif; ?>><?php echo
                    $side_label;
                    ?></div>
                ...
                <div class="custom-select">
                    <select class="border-style" data-side="<?php echo esc_attr($side); ?>">
                        <?php foreach ($styles as $style => $label) : ?>
                            ...
                            ?>
                            <option
                                value="<?php echo esc_attr($style); ?>" <?php selected($current_style, $style); ?>><?php echo esc_html($label); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="color-picker">
                    ...
                    ?>
                    <input type="text" class="color-select" data-side="<?php echo esc_attr($side); ?>"
                           value="<?php echo esc_attr($color); ?>" data-opacity="<?php echo esc_attr($opacity); ?>"/>
                    <a class="remove-color ti-close"
                       href="#" <?php echo ('' != $color || '' != $opacity) ? 'style="display:inline"' : ''; ?>
                       data-side="<?php echo esc_attr($side); ?>"></a>
                </div>
                ...
                ?>
                <input type="text" class="dimension-width border-width" data-side="<?php echo esc_attr($side); ?>"
        ...
        <div class="themify-customizer-brick collapse-same">
            ...
            <input id="<?php echo esc_attr($same_id); ?>" type="checkbox" class="same" <?php checked($same, 'same'); ?>
                   value="same"/>
