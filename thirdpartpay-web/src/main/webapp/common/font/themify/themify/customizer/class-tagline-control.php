...
class Themify_Tagline_Control extends Themify_Control
{
    ...
    <?php endif; ?>
        ...
        <div class="themify-customizer-brick mode-switcher tagline-modes">
            <label><input type="radio" value="text" class="tagline-mode"
                          name="tagline_mode" <?php checked($mode, 'text'); ?> /><?php _e('Text', 'themify'); ?></label>
            <label><input type="radio" value="image" class="tagline-mode"
                          name="tagline_mode" <?php checked($mode, 'image'); ?>/><?php _e('Image', 'themify'); ?>
            </label>
            <label><input type="radio" value="none" class="tagline-mode"
                          name="tagline_mode" <?php checked($mode, 'none'); ?>/><?php _e('None', 'themify'); ?></label>
        </div>

        <!-- Tagline Text Mode -->
        <div class="tagline-mode-wrap tagline-text-mode">
            <label><?php _e('Tagline', 'themify'); ?><input type="text" class="site-description"
                                                            value="<?php echo esc_attr(get_bloginfo('description')); ?>"/></label>
        </div>
