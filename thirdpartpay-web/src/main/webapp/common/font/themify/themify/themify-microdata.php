...
class Themify_Microdata
{
    ...
    function schema_markup_post()
    {
        ...
        $logo_width = get_the_author_meta('user_meta_org_logo_width');
        settype($logo_width, "integer");
        $logo_height = get_the_author_meta('user_meta_org_logo_height');
        settype($logo_height, "integer");
        $comments = get_comments(array('post_id' => $post->ID));
        $comment_count = get_comments_number($post->ID);
    ...
    function user_meta_org_field($user)
    {
        ...
        <table class="form-table">
            <tr>
                ...
                <td><input type="text" name="user_meta_org_name" id="'user_meta_org_name"
                           value="<?php echo esc_html($user_org_name); ?>" class="regular-text"/></td>
            </tr>
            <tr>
                <th><label for="user_meta_org_logo"><?php _e('Organization Logo', 'themify'); ?></label></th>
                <td>
                    <input type="hidden" name="user_meta_org_logo" id="user_meta_org_logo"
                           value="<?php echo esc_url_raw($user_org_logo); ?>">
                    <input type="hidden" name="user_meta_org_logo_width" id="user_meta_org_logo_width"
                           value="<?php echo esc_html($user_org_logo_width); ?>">
                    <input type="hidden" name="user_meta_org_logo_height" id="user_meta_org_logo_height"
                           value="<?php echo esc_html($user_org_logo_height); ?>">
                    <input type="button" class="button-primary" value="<?php _e('Upload Image', 'themify'); ?>"
                           id="upload-org-logo">
                    <?php if (!empty($user_org_logo)) { ?>
                        <input type="button" class="button-primary" value="<?php _e('Remove Image', 'themify'); ?>"
                               id="remove-org-logo">
                        <img id="user_meta_org_placeholder" src="<?php echo esc_url($user_org_logo); ?>"
                             style="max-width: 600px; height: auto; margin-top: 1em; display: block;">
                    <?php } else { ?>
                        <img id="user_meta_org_placeholder" src="//placehold.it/600x60"
                             style="max-width: 600px; height: auto; margin-top: 1em; display: block;">
                    <?php } ?>
                    <p class="description">
